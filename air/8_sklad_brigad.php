...
<div class="header-block">
    <h>Отримати склад бригад зазначеної ділянки, цеху.
    </h>
</div>
...
if (
    ...
    $query = " 
    select  department_name, area_name, brigada_id, staff_full_name, staff_position, staff_seniority
    from aviator.staff
    left join department_areas da on da.area_id = staff.area_id
    left join departments d on d.department_id = da.department_id
    where brigada_id is not null " . $cond . " order by 1,2,3";


    $columns = array('department_name', 'area_name', 'brigada_id', 'staff_full_name', 'staff_position', 'staff_seniority');
